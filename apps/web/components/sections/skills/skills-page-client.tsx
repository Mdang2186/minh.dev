        ...
        <Container>
          ...
          >
            <motion.div variants={itemVariants} className="text-center mb-16 sm:mb-20">
              ...
              <h1 className="text-3xl sm:text-5xl md:text-6xl font-extrabold tracking-tight mb-3 sm:mb-4 text-slate-900">
                Skills <span className="text-slate-300">&</span>{" "}
                <span className="text-cyan-600">Tools</span>
              </h1>
              <p className="text-base sm:text-lg text-slate-500 max-w-2xl mx-auto leading-relaxed font-medium px-4">
                ...
                groups.map((group, index) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className="p-5 sm:p-8 lg:p-10 rounded-[1.5rem] sm:rounded-3xl bg-white border border-slate-200 shadow-sm hover:shadow-md hover:border-slate-300 transition-all duration-300"
                    >
                      <div className="flex flex-col md:flex-row gap-5 lg:gap-12">
                        <div className="md:w-1/3 flex-shrink-0">
                          <div className="flex items-center gap-3 sm:gap-4 mb-2 sm:mb-3">
                            <div className="p-2.5 sm:p-3 bg-slate-100 border border-slate-200 rounded-xl sm:rounded-2xl text-slate-800">
                              <Icon className="w-5 h-5 sm:w-6 sm:h-6" />
                            </div>
                            <h3 className="text-lg sm:text-xl font-bold text-slate-900">{group.title}</h3>
                          </div>
                          <p className="text-[13px] sm:text-sm text-slate-400">{group.skills.length} kỹ năng</p>
                        </div>
                          ...
                          {group.skills.map((skill) => {
                            ...
                            return (
                              <button
                                ...
                                className="group/skill flex items-center gap-2 sm:gap-2.5 px-3 sm:px-4 py-2 sm:py-2.5 rounded-xl sm:rounded-2xl border border-slate-200 bg-slate-50 hover:bg-white hover:border-cyan-300 hover:shadow-md transition-all duration-200 cursor-pointer select-none"
                              >
                                {tech ? (
                                  <tech.icon
                                    style={{ color: tech.color }}
                                    className="w-4 h-4 sm:w-5 sm:h-5 flex-shrink-0 transition-transform duration-200 group-hover/skill:scale-110"
                                  />
                                ) : (
                                  <Code2 className="w-4 h-4 sm:w-5 sm:h-5 text-slate-400 flex-shrink-0" />
                                )}
                                <span className="text-[13px] sm:text-sm font-semibold text-slate-700 group-hover/skill:text-cyan-700 transition-colors whitespace-nowrap">
                                  {skill.name}
                                </span>
                                {lvl && (
